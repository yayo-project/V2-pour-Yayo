...
- NO contact details are ever public — no phone, WhatsApp or email visible to anyone browsing, from dealers OR agencies. Before an accepted offer, all contact happens through in-app chat only, both directions. AFTER an accepted offer, contact opens by stages — see "Contact, orders and money (locked)" below. Numbers exist in the backend for admin, never on a public page.
...
- MESSAGING MUST MATCH OR BEAT WHATSAPP (founder rule, 2026-08-23). Chat is where the deal actually happens, so it is never the page that gets the cheap version. Every chat surface — buyer inbox (messages.html), car page, agency page, dealer/agency dashboard — carries the SAME composer: text, photo, document, voice note with the sender reviewing the transcript, read ticks, live arrival, translation. If a capability ships on one surface it ships on all four the same day; a buyer must never find two different Yayos depending on which page they opened. On a phone the thread is full-screen with a back arrow (never a cropped list stacked on a short thread), the composer stays reachable at the bottom, and text inputs are 16px so iOS does not zoom.
...
## Contact, orders and money (locked — decided 2026-08-20)
Full reasoning, screens and the weaknesses that remain: https://claude.ai/code/artifact/7fbe5981-ac3c-4858-bfc8-1ada68ea99a9
- YAYO IS NOT A REGISTERED COMPANY YET. Until it is: never ask a dealer or agency for bank details, never store ID/passport numbers, never bill or collect anything, never process a payment. Record what WOULD be owed on every accepted offer/quote, marked not-billable — that becomes the invoice history and the sales argument later. Target: registered before the dealer promos expire (20 Oct – 13 Nov 2026).
- THE TRADE LICENCE IS NEVER SHOWN TO ANYONE. The scan stays in the private "licenses" bucket, admin-only — never publish it, never show licence data to buyers. During verification the ADMIN (never the dealer) records legal name, trading name, licence number, authority, expiry, registered address. Those fields exist to VERIFY, not to display.
- LICENCE EXPIRY IS MONITORED automatically (a third scheduled function alongside weekly-stats and reconcile-limits): dealer emailed at 60 and 30 days, admin countdown at 7, badge falls automatically on expiry — the badge only, never the listings. An expired licence makes that dealer's documents "unverifiable".
- DOCUMENT CHECKS HAVE THREE OUTCOMES, never two: matches (delivered, marked verified) / unreadable (delivered, marked unverifiable, admin notified) / different company (held, dealer + admin notified). Blocking on "unreadable" would kill honest sales daily — a phone photo in a dim showroom is usually unreadable, and UAE trading names differ from legal names almost always.
- BANK ACCOUNT NAME CHECK, WITHOUT COLLECTING ANYTHING: Yayo stores no IBAN. When a dealer types his own payment details in chat, compare only the account-holder NAME against the verified company name and WARN the buyer on mismatch ("ne payez pas avant d'avoir vérifié"). A warning, never a block. Structured payment details come only after registration.
- AN ORDER IS CREATED BY AN ACCEPTED PRICED OFFER — the seller offers (price + validity), the buyer accepts. Never a buyer-only "I want to buy" button: a dealer would just tell the buyer to press it. An order holds OPTIONAL LINES (car / transport / later spare parts), so car-only, transport-only (a car bought outside Yayo) and both all work. Transport arranged later JOINS the existing order, never creates a second one.
- "MES COMMANDES" appears in the buyer's account only once an order exists — no empty menu. Newest first. Each row shows what the order is WAITING FOR, and "À vous de jouer" when it's the buyer's turn. Transport lines use the EXISTING shipments table (7 steps, shipment_events) — it is already built and unused; do not rebuild it. Steps outside Yayo are shown dashed and named honestly.
- CONTACT OPENS IN STAGES, never all at once: (0) discussion — nothing, both directions, seller sees only "Acheteur #CODE · City"; (1) offer accepted — the BUYER receives the seller's business identity, nothing flows back; (2) the buyer fills the shipping form HIMSELF — name, address, phone go only to the parties that need them, because export documents require a named consignee. The form must accept TWO people (buyer ≠ consignee) or the diaspora market cannot use it.
- AGENCIES PAY A FLAT FEE PER ACCEPTED BOOKING, NEVER A PERCENTAGE (~$175; a % invites under-declaring the freight price). Nothing is released to an agency before acceptance inside Yayo. Prepaid credits after registration. State the future price at the FIRST conversation even while it's free ("founding partner, free until X, then $175") — introducing a fee later loses the agency. A buyer may always use his own forwarder: keep the order, lose that commission, and invite the forwarder to join.
- CONTACT DETAILS ARE FILTERED FROM MESSAGES AND IMPORTED DESCRIPTIONS before an order exists — text AND voice (every voice note is transcribed for translation anyway, so it passes the same filter; convert spelled-out digits first, and catch intent like "on continue sur WhatsApp"). Never block silently: tell the sender why, in their language. The filter lifts once an offer is accepted.
- VOICE MESSAGES: the SENDER reviews the transcript before sending ("Yayo a compris ceci — c'est bien ce que vous avez dit ?"). Numbers are copied verbatim, never translated. Unintelligible passages print as [passage inaudible] — the model must never invent. The original audio always travels with the translation.
